...
#A) Teorija
 #1) Zato što je RAM samo trenutna radna memorija te podaci spremljeni u njemu će biti izgubljeni nakon što program završi raditi, a u trajnoj memoriji podatci su trajno spremljeni te ih možemo pregledavati i uređivati makar je program završio raditi.
 #2) CSV format ima jednostavnu strukturu, sprema podatke odvojene zarezima, dokle XML ima složeniju hijerarhijsku strukturu nalik HTML jeziku. Također, CSV datoteke su znatno manje i učinkovitije u pogledu prostora za pohranu u usporedbi s XML-om, XML datotetke zauzimaju više prostora zbog ponavljajućih oznaka za svaki podatak.
 #3) Konstrukcija with open(...) as f: automatski otvara i zatvara datoteku. Glavna prednost toga je da automatski zatvara datoteku čim kod prestane sa radom, bez obzira na to kako je blok koda napušten (normalnim završetkom ili zbog iznimke/greške) što čini kod i spremanje koda sigurnijim.
 #4) Listbox treba očistiti prije učitavanja podataka iz datoteke zato da se stari podaci ne preklapaju/miješaju sa novima.
 #5) DictReader automatski pretvara redove u rječnike s ključevima koji su nazivi stupaca, dokle kod split() se treba znati točan redoslijed stupaca. DictReader omogućuje čitljiviji kod i ne ovisi o redoslijedu stupaca, zato samo DictReader pravilno obrađuje podatke s zarezima. 
